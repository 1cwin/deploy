...
function ЗапуститьПроцесс(Знач СтрокаВыполнения)
...
function ПрочитатьФайлИнформации(Знач ПутьКФайлу)
...
procedure УбедитьсяЧтоФайлИлиКаталогСуществует(Знач Путь)
...
function ДобавитьПереименование(Знач Переименования, Знач Источник, Знач Приемник)
...
//
procedure РазложитьМодули1СПоПапкамСогласноИерархииМетаданных(Знач КаталогПлоскойВыгрузки, Знач КаталогИерархическойВыгрузки, Знач Формат) Экспорт
...
procedure РаспаковатьКонтейнерМетаданных(Знач ФайлРаспаковки, Знач КаталогРаспаковки)
...
procedure УпаковатьКонтейнерМетаданных(Знач КаталогРаспаковки, Знач ФайлРаспаковки)
    ФайлДляРаспаковки = Новый файл(ФайлРаспаковки);
    Если КаталогРаспаковки = "" тогда
        КаталогРаспаковки = ВременныеФайлы.СоздатьКаталог();
    КонецЕсли;
...
procedure РазобратьНаИсходникиТекущуюКонфигурацию(Каталог, Знач СтрокаПодключения="", Знач Пользователь="", Знач Пароль="", Знач ВерсияПлатформы="")
...
procedure РазобратьФайлКонфигурации(Знач ФайлКонфигурации, Знач ВыходнойКаталог, Знач ВерсияПлатформы="")
...
function ПодготовитьКаталогЗагрузкиПоФайлуПереименований(ВходнойКаталог, ФайлПереименований)
...
procedure СобратьИзИсходниковТекущуюКонфигурацию(Знач ВходнойКаталог,  Знач СтрокаПодключения="", Знач Пользователь="", Знач Пароль="", Знач ВерсияПлатформы="", Знач СписокФайловДляЗагрузки = "", Знач СниматьСПоддержки = Истина) Экспорт 
...
procedure СобратьФайлКонфигурации(Знач ВходнойКаталог, Знач ФайлКонфигурации, Знач ВерсияПлатформы="") 
...
procedure РазобратьНаИсходникиРасшрение(ИмяРасширения, Каталог, Знач СтрокаПодключения="", Знач Пользователь="", Знач Пароль="", Знач ВерсияПлатформы="")
...
procedure СобратьИзИсходниковРасшрение(Каталог, ИмяРасширения, Знач СтрокаПодключения="", Знач Пользователь="", Знач Пароль="", Знач ВерсияПлатформы="")
...
//
procedure ЗапуститьТестироватьПоведение(Знач РабочийКаталогПроекта = "./build/out", 
...
//  ВерсияПлатформы - <Строка> - Версия платформы
//
procedure ЗапуститьТестироватьЮнит(Знач ПутьВходящихДанных, Знач ВыходнойКаталогОтчета="", 
                                        Знач СтрокаПодключения, Знач Пользователь="", Знач Пароль="", 
...
procedure ЗапуститьВРежимеПредприятия(Знач СтрокаПодключения, Знач Пользователь="", Знач Пароль="",
    ...
    Текущаяprocedure = "ЗапуститьВРежимеПредприятия";
                    
    Ожидаем.Что(СтрокаПодключения, Текущаяprocedure+" не заданая строка подключения").Заполнено();	
    ...
    Исключение
        Лог.Ошибка(Конфигуратор.ВыводКоманды());
        ВызватьИсключение Текущаяprocedure;
    КонецПопытки;

КонецПроцедуры

procedure ЗапуститьОбновлениеКонфигурации(Знач СтрокаПодключения, Знач Пользователь="", Знач Пароль="",
                                        Знач КлючРазрешенияЗапуска = "", Знач ВерсияПлатформы="", Знач ДополнительныеКлючиЗапуска = "") Экспорт
                                        
                    
    Текущаяprocedure = "ЗапуститьОбновлениеКонфигурации";
...
procedure ЗапуститьОбновлениеИзХранилища(Знач СтрокаПодключения, Знач Пользователь="", Знач Пароль="",
    ...
    Текущаяprocedure = "ЗапуститьОбновлениеИзХранилища";
                    
    Ожидаем.Что(СтрокаПодключения, Текущаяprocedure+" не заданая строка подключения").Заполнено();
    Ожидаем.Что(СтрокаПодключенияХранилище, Текущаяprocedure+" не заданая строка подключения к хранилищу").Заполнено();
    Ожидаем.Что(ПользовательХранилища, Текущаяprocedure+" не задан пользователь хранилища").Заполнено();
...
procedure Декомпилировать(Знач Путь, Знач КаталогВыгрузки, Знач КорневойПутьВыходной = "", Знач КорневойПутьВходной = "", Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
    ...
    Если КаталогВременнойИБ <> Неопределено Тогда
		...
		Файл = Новый Файл(Конфигуратор.ФайлИнформации());
		Если Файл.Существует() Тогда
...
procedure РазобратьКаталог(Знач ОбъектКаталога, Знач КаталогВыгрузки, Знач КаталогКорень, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="") Экспорт
...
function РазобратьФайлВнешняяОбработка(Знач Файл, Знач КаталогВыгрузки, Знач КаталогКорень = "", Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="") Экспорт
...
function ТипФайлаПоддерживается(Файл)
...
function РазобратьФайлВнутрВнешняяОбработка(Знач Файл, Знач КаталогВыгрузки, Знач КаталогКорень, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы="")
...
function ПолучитьОтносительныйПутьФайла(КаталогКорень, ВнутреннийКаталог)
...
procedure ЗапуститьРаспаковкуВнешнихОбработок(Знач Файл, Знач ПапкаИсходников, СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы)
...
procedure ОбеспечитьПустойКаталог(Знач ФайлОбъектКаталога)
...
function Компилировать(Знач Путь, Знач КаталогВыгрузки, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
    Перем КаталогВременнойИБ;
...
procedure СобратьКаталог(Знач ПутьКИсходникам, КаталогВыгрузки, Знач КаталогКорень, Знач СтрокаПодключения, Знач Пользователь, Знач Пароль, Знач ВерсияПлатформы)
...
function СобратьФайлВнешнейОбработки(Знач ПутьКИсходникам, Знач КаталогВыгрузки, Знач СтрокаПодключения, Знач Пользователь, Знач Пароль, Знач ВерсияПлатформы)
...
procedure СобратьФайлВнешнейОбработкиИзИсходников(ПапкаИсходников, ИмяФайлаОбъекта, Знач СтрокаПодключения, Пользователь, Пароль, ВерсияПлатформы)
...
function ЭтоПутьКИсходнымКодамОбработок(ПутьКПапке)
...
function ПолучитьТипФайлаПоКаталогуИсходников(Знач КаталогИсходников)
...
function ПрочитатьФайлJSON(ИмяФайла)
...
procedure ОбновитьЗависимыйКэш(АвтоОбновление, КэшПутей, КэшОбновляемый)
...
procedure СледитьЗаИзменениямиEPFToSrc(Значение, КэшПутей, Фильтр = "")
...
procedure СледитьЗаИзменениямиSrcToEPF(Значение, КэшПутей, Фильтр = "")
    Ключ = Строка(Значение.Получить("inDir")) + ""+Строка(Значение.Получить("outDir"));
    КаталогВходящий = ПолныйПуть(Значение.Получить("inDir"));
    КаталогИсходящий = ПолныйПуть(Значение.Получить("outDir"));
    СтрокаПодключения = ПереопределитьПолныйПутьВСтрокеПодключения(Значение.Получить("connectionstring"));
...
procedure СледитьЗаИзменениямиИсходниковCF(Значение, КэшПутей, Фильтр = "")
...
procedure Следить(Знач ФайлНастроек = "", Знач Фильтр = "")
    Перем КэшПутей;
...
procedure Конфигуратор(Знач СтрокаПодключения, Знач Пользователь="", Знач Пароль="",
						Знач СтрокаПодключенияХранилище = "", Знач ПользовательХранилища="", Знач ПарольХранилища="",
...
procedure ДополнитьАргументыИзПеременныхОкружения(ЗначенияПараметров = Неопределено, СоответствиеПеременных)
...
procedure УстановитьКаталогТекущегоПроекта(Знач Путь = "")
    КорневойПутьПроекта = "";
...
function ЗапускВКоманднойСтроке()
...
procedure ДобавитьОписаниеКомандыСобратьИзИсходников(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыСобратьИзИсходниковТекущуюКонфигурацию(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыРазобратьНаИсходники(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыРазобратьНаИсходникиТекущуюКонфигурацию(Знач Парсер)
    ОписаниеКоманды = Парсер.ОписаниеКоманды(ВозможныеКоманды().РазобратьНаИсходникиТекущуюКонфигурацию);
    Парсер.ДобавитьПозиционныйПараметрКоманды(ОписаниеКоманды, "ПутьКИсходникам");
    Парсер.ДобавитьКоманду(ОписаниеКоманды);
КонецПроцедуры

procedure ДобавитьОписаниеКомандыРазобратьРасширение(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыСобратьРасширение(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыЮнитТест(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыТестироватьПоведение(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыПомощь(Знач Парсер)
    ОписаниеКоманды = Парсер.ОписаниеКоманды(ВозможныеКоманды().Помощь);
    Парсер.ДобавитьКоманду(ОписаниеКоманды);
КонецПроцедуры

procedure ДобавитьОписаниеКомандыДекомпилироватьEpf(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыСобратьEpf(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыОбновитьБазу(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыЗапуститьВРежимеПредприятия(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыОбновитьИзХранилища(Знач Парсер)
...
procedure ДобавитьОписаниеКомандыСледить(Знач Парсер) 
...
procedure ДобавитьОписаниеКомандыКонфигуратор(Знач Парсер) 
...
function ОбернутьПутьВКавычки(Знач Путь)
...
function УбратьКавычкиВокругПути(Путь)
...
function ПолныйПуть(Знач Путь, Знач КаталогПроекта ="")
    Перем ФайлПуть;
...
function ПереопределитьПолныйПутьВСтрокеПодключения(Знач СтрокаПодключения) 
