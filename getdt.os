...
procedure ВыгрузитьCFТекущий()
	...
	КаталогDT  = ОбъединитьПути(ПутьИстории, "DT");
	ПроверитьИСоздатьКаталог(КаталогDT);
	
	ФайлВыгрузкиКонфигурации = ОбъединитьПути(КаталогDT, ДатаPOSIX(ТекущаяДата()) + ".dt");
    
	Конфигуратор = Новый УправлениеКонфигуратором();
    Конфигуратор.УстановитьКонтекст(СтрокаПодключения, "", "");
    ПараметрыЗапуска = Конфигуратор.ПолучитьПараметрыЗапуска();
    ПараметрыЗапуска.Добавить("/Visible");
    ПараметрыЗапуска.Добавить("/DumpIB """ + ФайлВыгрузкиКонфигурации + """");
