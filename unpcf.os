...
function ОбработатьФайлEPF(Знач Файл, Знач КаталогОбработки)
	...
	ИмяОбработки = Новый Файл(КаталогОбработки).Имя;
	НовоеИмяФайла = ОбъединитьПути(КВФ, ИмяОбработки)+ Файл.Расширение;
	КопироватьФайл(Файл.ПолноеИмя, НовоеИмяФайла); 
	
	Ошибка = Ложь;
	Попытка
		Runner.Декомпилировать(НовоеИмяФайла, КаталогИсходниковEPF,, СтрокаПодключения);
		ВыполнитьКоммит(КаталогИсходниковEPF, "" + ИмяОбработки + " (" + Файл.Имя + ")", Файл.ДатаИзменения);
	Исключение
		Ошибка = Истина;
	КонецПопытки;
