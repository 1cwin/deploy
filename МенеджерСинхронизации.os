...
//
procedure РазобратьФайлКонфигурации(Знач ФайлКонфигурации, Знач ВыходнойКаталог, Знач Формат) Экспорт
...
//
procedure ВыгрузитьМодулиКонфигурации(Знач ФайлКонфигурации, Знач КаталогПлоскойВыгрузки, Знач Формат) Экспорт
...
function ПолучитьМенеджерКонфигуратора()
...
procedure ВыполнитьКомандуКонфигуратора(Знач Конфигуратор, Знач ПараметрыЗапуска)
...
function МассивНеКритическихОшибок()
...
function ПроверитьОписаниеОшибкиНаКритичность(Знач ОписаниеОшибки)
...
function ЭтоНативнаяИерархия(Знач КаталогПроверки)
    КаталогЯзыки = Новый Файл(ОбъединитьПути(КаталогПроверки, "Languages"));
    Возврат КаталогЯзыки.Существует();
КонецФункции

procedure ОчиститьЦелевойКаталогВыгрузки(Знач КаталогИерархическойСтруктурыМодулей)
...
procedure ОбработатьКаталогРезультатаВыгрузки(Знач Файл, Знач КаталогИерархическойСтруктурыМодулей, Знач Переименования)
...
function СкорректироватьИмяФайлаМетаданных(Знач ИмяФайла)
...
procedure СократитьДлинуИмениФайла(ИмяФайла, Знач КаталогИерархическойСтруктурыМодулей)
...
//
procedure ВыполнитьКоммитГит(Знач КаталогРабочейКопии, Знач Комментарий, Знач Автор = "", Знач Дата=Неопределено) Экспорт
	...
	КомандаКоммита = "git commit -a --file="""+ИмяФайлаКомментария+""" >"+ИмяФайлаЛогаКоммита;
...
// Cтандартная procedure git push
//
function ВыполнитьGitPush(Знач ЛокальныйРепозиторий, Знач УдаленныйРепозиторий, Знач ИмяВетки = Неопределено) Экспорт
...
// Cтандартная procedure git pull
//
function ВыполнитьGitPull(Знач ЛокальныйРепозиторий, Знач УдаленныйРепозиторий, Знач ИмяВетки) Экспорт
...
function СоздатьКомандныйФайл(Знач Путь = "")

	Файл = Новый КомандныйФайл();
	Файл.Открыть(Путь);
	
	Если ТихийРежим = Истина Тогда
...
procedure ДобавитьВКомандныйФайл(Знач ДескрипторКомандногоФайла, Знач Команда)
...
function ЗакрытьКомандныйФайл(Знач ДескрипторКомандногоФайла)
...
//
procedure УстановитьАвторизациюВХранилищеКонфигурации(Знач Логин, Знач Пароль, Знач ВерсияПлатформы = "")
...
//
function ПрочитатьТаблицуИсторииХранилища(Знач ФайлХранилища) Экспорт
...
//
function ПрочитатьТаблицуПользователейХранилища(Знач ФайлХранилища) Экспорт
...
//
function ПрочитатьИзХранилищаИсториюКоммитовСАвторами(Знач ФайлХранилища) Экспорт
...
//
procedure ДополнитьТаблицуХранилищаИнформациейОСигнатуреПользователяВГит(Знач ТаблицаХранилища, Знач КаталогРепо) Экспорт
...
function КонвертироватьТаблицуВерсийИзФорматаБД(Знач ТаблицаБД)
...
function КонвертироватьТаблицуПользователейИзФорматаБД(Знач ТаблицаБД)
...
function НоваяТаблицаИсторииВерсий()
...
function НоваяТаблицаПользователейХранилища()
...
procedure ДополнитьТаблицуВерсийИменамиАвторов(Знач ТаблицаВерсий, Знач ТаблицаПользователей)
...
//
procedure СформироватьПервичныйФайлПользователейДляGit(Знач ИмяФайлаХранилища, Знач ВыходнойФайл) Экспорт
...
procedure ЗаписатьТаблицуПользователейВФайлАвторовGit(Знач ТаблицаПользователейХранилища, Знач ВыходнойФайл, Знач ДоменПочтыДляGit)
...
//
function КлонироватьРепозитарий(Знач КаталогЛокальнойКопии, Знач URLРепозитария) Экспорт
...
//
function ИнициализироватьРепозитарий(Знач КаталогЛокальнойКопии) Экспорт
...
//
function ПроверитьНаличиеРепозитарияГит(Знач КаталогЛокальнойКопии) Экспорт
	Батник = СоздатьКомандныйФайл();
	Лог.Отладка(КаталогЛокальнойКопии);
...
//
function ТребуетсяСинхронизироватьХранилищеСГит(Знач ФайлХранилища, Знач ЛокальныйКаталогГит) Экспорт
...
// Основная function синхронизации. Перемещает в git набор коммитов хранилища 1С
//
procedure СинхронизироватьХранилищеКонфигурацийСГит(Знач КаталогРабочейКопии, 
		Знач ФайлХранилища, 
...
function ОпределитьМаксимальнуюВерсиюСУчетомОграниченияСверху(Знач ТаблицаИсторииХранилища, Знач ТекущаяВерсия, Знач МаксимальнаяВерсия)
...
function НомерСинхронизированнойВерсии(Знач КаталогВыгрузки)
...
function ОпределитьМаксимальнуюВерсиюВХранилище(Знач ТаблицаИсторииХранилища)
...
procedure РазложитьМодулиПоНомеруВерсииХранилища1С(Знач КаталогРабочейКопии, Знач ФайлХранилища, Знач НомерВерсии, Знач Формат) Экспорт
...
procedure ПоНомеруВерсииСохранитьКонфигурациюСредствамиTool1CD(Знач ПутьКФайлуХранилища1С, Знач ВыходнойФайл, Знач НомерВерсииХранилища)
...
function ИмяФайлаВыгрузкиВерсииХранилища(Знач Каталог, Знач НомерВерсии)
	Возврат ОбъединитьПути(Каталог,  "v"+Строка(НомерВерсии) +".cf");
КонецФункции

procedure ВывестиТекстФайла(Знач ИмяФайла, Знач Кодировка = Неопределено)
...
function СуффиксПеренаправленияВывода(Знач ИмяФайлаПриемника, Знач УчитыватьStdErr = Истина)
	Возврат "> """ + ИмяФайлаПриемника + """" + ?(УчитыватьStdErr, " 2>&1", "");
КонецФункции

function СобратьКоманднуюСтроку(Знач ПереченьПараметров)
...
function ПолучитьТекстФайла(ИмяФайла, резТекстФайла = "")
...
function ОбернутьВКавычки(Знач Строка);
	Возврат """" + Строка + """";
КонецФункции
...
function ИмяФайлаАвторов() Экспорт
	Возврат "AUTHORS";
КонецФункции

function ИмяФайлаВерсииХранилища() Экспорт
	Возврат "VERSION"
КонецФункции

procedure ЗаписатьФайлВерсийГит(Знач КаталогФайлаВерсий, Знач Версия = "") Экспорт
...
function ПрочитатьФайлВерсийГит(Знач ПутьКФайлуВерсий)
...
function ПрочитатьФайлАвторовГитВТаблицуПользователей(Знач ПутьКФайлуАвторов) Экспорт
...
procedure УстановитьРежимУдаленияВременныхФайлов(ПарамУдалятьВременныеФайлы) Экспорт
...
procedure УдалитьВременныеФайлыПриНеобходимости(Знач ПутьКФайлу = "")
...
procedure ДополнитьМассив(Знач Приемник, Знач Источник)
...
function ЗапуститьПриложениеИДождатьсяЗавершения(Знач СтрокаЗапуска, Знач ТекущийКаталог = "\.")
...
procedure ЗавершитьПроцесс_TGitCache_exe()
	Если ЭтоWindows Тогда
